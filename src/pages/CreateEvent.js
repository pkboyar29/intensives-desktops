...
import { convertDateFormat, convertBackDateFormat, filterNotIncludeObjectsByNames, filterIncludeObjectsByNames } from '../utils'
         ...
         await Promise.allSettled([
            ...
            if (response[4].status === 'fulfilled') {
               ...
               setDateStart(convertBackDateFormat(response[4].value.data.start_dt))
                  (response[4].value.data.finish_dt) ? setDateEnd(convertBackDateFormat(response[4].value.data.finish_dt)) : setDateEnd('')
               setCommands(filterIncludeObjectsByNames(response[0].value.data.results, response[4].value.data.commands))
               setResponseCommands(filterNotIncludeObjectsByNames(response[0].value.data.results, response[4].value.data.commands))
               setTeachers(response[4].value.data.teachers_command.map((teacher) => {
                  return { id: teacher }
               }))
            }
         })
                     ...
                     <form>
                        ...
                        <div className='element-list-input column-container'>
                           ...
                           <div className='flex flex-wrap'>
                              {commands.length > 0 ? (
                                 commands.map((item) => (
                                    <div key={item.name} className='ml-4 text-sm selectedInList'>
                                       {item.name}
                                    </div>
                                 ))
                              ) : (
                           ...
                           <div className='align-center flex'>
                              ...
                              {(commands.length > 0) ? null : <span style={{ height: 'max-content', marginLeft: '20px' }} className='text-[#6B7280]'>Выберите команды</span>}
                           </div>
                        </div>
                        ...
                        <div className='element-list-input column-container'>
                           ...
                           <div className='flex flex-wrap'>
                              {teachers.length > 0 ? (
                                 teachers.map((item) => (
                                    <div key={item.id} className='ml-4 text-sm selectedInList'>
                                       {item.name}
                                    </div>
                                 ))
                              ) : (
                           ...
                           <div className='align-center flex'>
                              ...
                              {(commands.length > 0) ? null : <span style={{ height: 'max-content', marginLeft: '20px' }} className='text-[#6B7280]'>Выберите участвующив в мероприятии преподавателей</span>}
                           </div>
                        </div>
                        <div className='element-list-input'>
                        ...
                        <InputRadioDescription setTypeProp={setTypeScore} descriptionProp={'Тип оценивания'} />
