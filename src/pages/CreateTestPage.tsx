...
const CreateTestPage: FC = () => {
  ...
  const DraggableQuestion = ({
    ...
    const [, drop] = useDrop({
      accept: ItemType,
      // hover больше не меняет порядок
  ...
  // Функция для определения touch-устройства (современно и безопасно)
  ...
  // Выбор backend и backendOptions для DndProvider (важно: useMemo!)
  const dndBackend = useMemo(() => (isTouchDevice() ? TouchBackend : HTML5Backend), []);
  const dndBackendOptions = useMemo(() => (isTouchDevice() ? { enableMouseEvents: true } : undefined), []);
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          {questions && (
            <div className="mb-4">
              ...
              />
              {questionsError && (
                <div className="mt-1 text-base text-red ">{questionsError}</div>
              )}
              ...
              {selectedQuestions.length > 0 && (
                <div className="mt-6">
                  ...
                  <DndProvider backend={dndBackend} backendOptions={dndBackendOptions}>
                    {selectedQuestions
