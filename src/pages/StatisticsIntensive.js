...
import { LineChart, Tooltip, CartesianGrid, Line, XAxis, YAxis } from 'recharts'
...
const StatisticsIntensive = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				await PostService.getStatisticsIntensiv(localStorage.getItem('id'))
					.then((response) => {
			...
			<div className='main-block'>
				<div className='center-block'>
					<div className='list-content column-container'>
						<div className='title'>
							<div className='font-32'>Статистика за интенсив {responseStatInt[0]?.intensive}</div>
						</div>
						<div className='font-14'>Оценки участников за интенсив</div>
						{
							(statisticsData)
								? <LineChart width={900} height={400} data={statisticsData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
									<XAxis dataKey='name' />
									<YAxis dataKey='score' />
									<Tooltip />
									<CartesianGrid stroke='#f5f5f5' />
									<Line type="type'basis" dataKey='score' stroke='#1A5CE5' yAxisId={0} />
								</LineChart>
								: <Skeleton />
						}
					</div>
